...
module.exports = async function(deployer, network, accounts) {
  ...
  const time = new Date();
  const configDeadManSwitch = {
    filename: 'dead_man.wasm',
    fn : 'construct(string,address,uint256,uint256)',
    args: [
      ['El veneno que vamos a dar a Kuzco', 'string'],
      [accounts[0], 'address'],
  ...
  const addressDeadManSwitch = await deploySecretContract(configDeadManSwitch);
  console.log(`Secret Contract "${configDeadManSwitch.filename}" deployed at Enigma address: ${addressDeadManSwitch} From account ${configDeadManSwitch.from}`);
